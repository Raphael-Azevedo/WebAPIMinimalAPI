...
await new HttpClient().GetStringAsync("https://ron-swanson-quotes.herukoapp.com/v2/quotes"));
...
app.MapGet("/tarefas/concluida", async (AppDbContext db) => await db.Tarefas.Where(x => x.IsConcluida == true).ToListAsync());
